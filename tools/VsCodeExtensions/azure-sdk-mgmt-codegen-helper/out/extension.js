...
function activate(context) {
    ...
    let regenDisposible = vscode.commands.registerCommand("azure-sdk-mgmt-codegen-helper.regenerate-code", (context) => {
        ...
        codegenHelper_1.CodeGenHelper.generateCode(cg);
    });
    // The command has been defined in the package.json file
    // Now provide the implementation of the command with registerCommand
    // The commandId parameter must match the command field in package.json
    let regenLatestDisposable = vscode.commands.registerCommand("azure-sdk-mgmt-codegen-helper.regenerate-code-with-latest-spec", (context) => {
        const cg = createCodeGenContext(context.fsPath);
        if (!cg) {
            logger_1.Logger.logError(`Unexpected Error: Failed to initialize context from path ${context.fsPath}.`);
            return;
        }
        codegenHelper_1.CodeGenHelper.generateCodeWithLatestSpec(cg);
    });
    let regenApiFileDisposiable = vscode.commands.registerCommand("azure-sdk-mgmt-codegen-helper.regenerate-api-file", (context) => {
        const cg = createCodeGenContext(context.fsPath);
        if (!cg) {
            logger_1.Logger.logError(`Unexpected Error: Failed to initialize context from path ${context.fsPath}.`);
            return;
        }
        codegenHelper_1.CodeGenHelper.generateApiFile(cg);
    });
    let updateChangelogDisposiable = vscode.commands.registerCommand("azure-sdk-mgmt-codegen-helper.update-changelog", async (context) => {
        ...
        await codegenHelper_1.CodeGenHelper.updateChangeLog(cg);
    });
    let createNewServiceDisposiable = vscode.commands.registerCommand("azure-sdk-mgmt-codegen-helper.init-new-service", async (context) => {
        await codegenHelper_1.CodeGenHelper.initNewSdk(context.fsPath);
    });
